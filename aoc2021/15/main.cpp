...
void solve2(std::vector<std::string> &data) {
    // load the points 5 times larger with increment
    size_t width = data.at(0).length() * 5, height = data.size() * 5;
    size_t original_width = width / 5, original_height = height / 5;

    std::vector<point> points(width * height);
    for (size_t y = 0; y < height; y++) {
        for (size_t x = 0; x < width; x++) {
            size_t posX = x % original_width;
            size_t posY = y % original_height;
            int cost = (int)(data.at(posY).at(posX) - '0') 
                + (x / original_width) 
                + (y / original_height);
